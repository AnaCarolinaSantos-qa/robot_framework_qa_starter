...
class A11y(SeleniumLibrary):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def run_axe_and_save(self, output_dir: str, page_name: str = None, fail_on: str = "serious"):
        driver = self.driver
        axe = Axe(driver)
        axe.inject()
        results = axe.run()
        ...
        if fail_on and fail_on.lower() in order:
            ...
            if to_fail > 0:
                raise AssertionError(
                    f"A11y: {to_fail} violações >= {fail_on}. "
                    f"Resumo {counts} | arquivo: {json_path}"
                )
        return {"file": str(json_path), "violations_total": len(results.get("violations", [])), "by_impact": counts}

    @property
    def driver(self):
        return self._drivers.active_driver
